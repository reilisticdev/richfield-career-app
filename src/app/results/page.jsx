...
export default function ResultsScreen() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // 1. Returning User from Magic Link
        if (session) {
          const { data: dbData } = await supabase.from('student_leads').select('*').eq('email', session.user.email).single();
          if (dbData) {
            setUserData({ firstName: dbData.first_name, program: dbData.current_program });
        ...
        // 2. New User coming from the Gamified Quiz
        ...
        // Fetching the Roadmap from Python Backend
        ...
        try {
            ...
            if (dbId) await supabase.from('student_leads').update({ psych_scores: parsedVector, roadmap_result: aiData }).eq('id', dbId); 
            
        } catch (parseError) {
            console.error("Failed to parse JSON. Python server returned an HTML error page.", parseError);
            alert("The AI Architect encountered a server error. Please check your Python terminal.");
            setIsLoading(false); return;
        }
  ...
  const handlePivot = async (e) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:5000/api/pivot", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ scores: userVector, program: userData.program, dream_job: dreamJobInput }) });
      if (!res.ok) throw new Error("Server error on Pivot.");
      setPivotData(await res.json());
    } catch (err) { console.error(err); alert("Failed to generate pivot analysis. Check backend."); } finally { setIsActionLoading(false); }
  };

  const handlePostgrad = async (e) => {
    e.preventDefault(); setIsActionLoading(true);
    try {
      const res = await fetch("http://127.0.0.1:5000/api/postgrad", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ program: userData.program, postgrad_choice: postgradSelect }) });
      if (!res.ok) throw new Error("Server error on Postgrad.");
      setPostgradData(await res.json());
    } catch (err) { console.error(err); alert("Failed to generate postgrad ROI. Check backend."); } finally { setIsActionLoading(false); }
  };
  ...
  const handleChat = async (e) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:5000/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: msg, program: userData.program, scores: userVector }) });
      const data = await res.json(); setChatMessages(p => [...p, { role: "assistant", content: data.response }]);
    } catch { setChatMessages(p => [...p, { role: "assistant", content: "Connection error. Please try again." }]); } finally { setIsChatLoading(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 sm:p-8 font-sans pb-32 selection:bg-blue-600 selection:text-white">
      <div className="w-full max-w-3xl mx-auto">
        ...
        {/* Header Block */}
        <div className="text-center mb-10 bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-100 relative overflow-hidden">
              ...
              <div className="space-y-10">
                {['year_1', 'year_2', 'year_3'].map((year, i) => (
                  <div key={year} className="relative border-l-[6px] border-blue-100 pl-6 pb-2">
                    ...
                    {roadmapData?.roadmap?.[year]?.mandatory_major && (
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 hover:shadow-md transition-shadow">
                        <span className="font-black text-blue-600 text-xs uppercase tracking-widest block mb-3">Semester 1</span>
                        <p className="text-sm font-medium text-slate-700 whitespace-pre-line leading-relaxed">{roadmapData?.roadmap?.[year]?.semester_1}</p>
                      </div>
                      <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 hover:shadow-md transition-shadow">
                        <span className="font-black text-blue-600 text-xs uppercase tracking-widest block mb-3">Semester 2</span>
                        <p className="text-sm font-medium text-slate-700 whitespace-pre-line leading-relaxed">{roadmapData?.roadmap?.[year]?.semester_2}</p>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {activeTab === "pivot" && (
          <div className="bg-white rounded-[2rem] shadow-xl p-8 sm:p-10 animate-in fade-in duration-500">
            ...
            <p className="text-slate-500 font-medium mb-8">Enter a career that wasn't in your roadmap. The AI will calculate the feasibility of bridging the gap using your current Richfield degree and the prospectus data.</p>
        ...
        {activeTab === "postgrad" && (
          <div className="bg-white rounded-[2rem] shadow-xl p-8 sm:p-10 animate-in fade-in duration-500">
            ...
            <p className="text-slate-500 font-medium mb-8">Select a Richfield postgraduate path to calculate your career acceleration and salary multiplier based on the official 2026 syllabus.</p>
            
            <form onSubmit={handlePostgrad} className="flex flex-col sm:flex-row gap-4 mb-10">
              {/* --- STEP 3: EXPANDED DROPDOWN --- */}
      ...
      <div className="fixed bottom-6 right-6 sm:bottom-10 sm:right-10 z-50 flex flex-col items-end pointer-events-none">
        ...
        {isChatOpen && (
          <div className="w-[calc(100vw-3rem)] sm:w-[400px] bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-200 mb-4 pointer-events-auto flex flex-col h-[600px] max-h-[70vh] animate-in slide-in-from-bottom-4 zoom-in-95 duration-200 overflow-hidden">
            ...
            <div className="bg-slate-900 text-white p-5 flex justify-between items-center shadow-md z-10">
              ...
              <button onClick={()=>setIsChatOpen(false)} className="text-slate-400 hover:text-white transition-colors p-2">
            ...
            <div className="flex-1 p-5 overflow-y-auto space-y-4 bg-slate-50 flex flex-col">
              ...
              {isChatLoading && (
                <div className="bg-white border border-slate-200 text-blue-600 self-start p-4 rounded-2xl rounded-tl-sm shadow-sm flex gap-1">
            ...
            <form onSubmit={handleChat} className="p-4 bg-white border-t border-slate-100 flex gap-3 z-10">
              <input type="text" value={chatInput} onChange={e=>setChatInput(e.target.value)} 
                className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-blue-600 focus:bg-white outline-none font-medium text-slate-900 transition-all placeholder:text-slate-400" 
                placeholder="Ask about modules or careers..." />
              <button type="submit" disabled={isChatLoading} 
                className="bg-slate-900 text-white px-5 rounded-xl font-black text-sm hover:bg-slate-800 transition-colors disabled:opacity-50 shadow-md active:scale-95 flex items-center justify-center">
        ...
        {!isChatOpen && (
          <button onClick={()=>setIsChatOpen(true)} className="group relative flex items-center justify-center w-16 h-16 bg-blue-600 text-white rounded-full shadow-[0_10px_25px_rgba(37,99,235,0.4)] pointer-events-auto hover:bg-blue-700 hover:scale-105 active:scale-95 transition-all duration-300 border-2 border-white">
            ...
            <span className="absolute -top-2 -right-2 w-5 h-5 bg-red-500 border-2 border-white rounded-full flex items-center justify-center text-[10px] font-black animate-pulse">1</span>
          </button>
        )}
      </div>
